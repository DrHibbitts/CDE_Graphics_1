...
\item
A way to change the physical properties of the linkage, such as the rate at which the joints can move, any specified constraints on the joints, and the slow-in and slow-out movement of the linkage as it leaves its current position and approaches the target position respectively.
...
Inverting the Jacobian matrix is an expensive operation. Since the matrix is often not square, the pseudoinverse can be implemented
...
\item
Calculate the pseudoinverse of the Jacobian $J^+$, or alternatively the transpose $J^T$
...
At this stage we added the simulation code originally developed in Matlab to our two-dimensional model, as shown in Figure~\ref{fig:2Dpicture}. Instead of calculating the pseudoinverse and the error, only the transpose of the Jacobian is calculated. To allow user input, we draw a triangle which could be moved using the mouse, and was used to mark the goal position. The mouse position was transformed using an inverse orthogonal projection. \\
...
\begin{figure}[h]
\centering
\includegraphics[scale=0.4]{SpeedDiagram}
\caption{Diagram illustrating the proposed slow-in and slow-out method as the end-effect moves from its initial position to the goal.}
\label{fig:scaling}
\end{figure}
...
A number of directions remain for future work. We would like to experiment with different shading and lighting models to achieve a higher level of realism. Another area for further work would be to include a graphical user interface to provide the user with direct control over the simulation parameters, such as the joint stiffness, and the slow-in and slow-out motion, which are currently embedded in the code. It would be interesting to explore other nonlinear optimisation methods to solve the inverse kinematics problem, such as Newton's method which incorporates higher order derivatives. In addition, alternative ways of implementing the constraints could be investigated, for instance the method of Lagrange multipliers. Finally, we are interested in loading three-dimensional mesh objects and textures into OpenGL to replace the simplistic geometric shapes used in the current implementation. The number of end-effectors could be increased to build more complex linkages and even create underlying skeleton rigs for more interesting objects or character models that could be animated. 
